...
/**
 * Memory Manager class. This version uses an array in memory. This version
...
public class MemManager
{
    ...
    int poolSize;
    /**
     * one freeList per block size with each index representing a block size of
     * 2^k
     */
    ...
    public MemHandle insert(byte[] info)
    {
        ...
        // Finding the index of the freeList field to look for a FreeBlock
        // object
        ...
        while (!allocated)
        {
            ...
            else
            {
                ...
                release(
                    new MemHandle(newBlock.start, newBlock.size, info.length));
            }
        }
        ...
        while (i > k)
        {
            ...
            // Creating the block's buddy within itself, dividing the memory by
            // 2
    ...
    public void release(MemHandle h)
    {
        ...
        else
        {
            ...
            release(
                new MemHandle(merged.start, merged.size, h.getRecordSize()));
        }
    }
    ...
    /**
     * Checks if value is a power of two by subtracting 1 to flip the bit and
     * all bits below it, then AND'ing them, which would result in a 0 if value
     * is a power of 2
     * 
     * @param x
     ...
     * left while p is less than x (the value being evaluated). This will keep p
     * as a power of 2 and only stop when we find the next largest power of 2.
     * 
     * @param x
     ...
     * right of the parameter (dividing by 2) until it is equal to one, tracking
     * the number of times it took to shift to the right, then returning that
     * value representing k.
     * 
     * @param x
     * @return log2(x)
     */
    ...
    public String printBlocks()
    {
        ...
        for (int i = 0; i < freeLists.length; i++)
        {
            if (!freeLists[i].isEmpty())
            {
                ...
                ans.append(freeLists[i].getStartsSorted());
